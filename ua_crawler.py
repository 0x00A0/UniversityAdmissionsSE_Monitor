...
file_handler = logging.FileHandler(filename='UniversityAdmissions.txt', mode='a', encoding='utf8')
...
if __name__ == '__main__':
	...
	application_status=["","","",""]
	while(42):
		response = s.post('https://www.universityadmissions.se/intl/loginajax', headers=headers, params=params, verify=False)
		...
		response = s.get('https://www.universityadmissions.se/intl/mypages', headers=headers, verify=False)
		soup = BeautifulSoup(response.text, 'html.parser')
		if soup.head.title.text!="My applications - Universityadmissions.se":
			continue
		courses = soup.find_all("div", class_="course")
		for i,course in enumerate(courses):
			course_name=course.find("h3", class_="coursehead_desktop heading4 coursename moreinfolink").text.strip()
			course_uni=course.find("span", class_="appl_fontsmall").text.split(',')[1].strip()
